...
MNIST.py
--------
Executes a lightweight CNN experiment over multiple batch sizes, logs
metrics, emits per-batch classification reports, and saves plots.
"""
...
def get_mnist_dataloaders(batch_size: int, num_workers: int = 2, pin_memory: bool = True):
    """Construct normalized MNIST dataloaders."""
...
class SmallMNISTConvNet(nn.Module):
    """Slim CNN tailored for 1-channel MNIST digits."""
...
def train_model(
    ...
    *,
    ...
    """Train the MNIST CNN while recording metrics and reports."""
    model = SmallMNISTConvNet().to(device)
    ...
    def current_lr(epoch_idx: int) -> float:
        if warmup_epochs > 0 and epoch_idx < warmup_epochs:
            return learning_rate * float(epoch_idx + 1) / float(warmup_epochs)
        return learning_rate

    for epoch in range(epochs):
        start = time.time()
        model.train()
        lr_now = current_lr(epoch)
...
def plot_mnist_results(results_df: pd.DataFrame, histories: dict, output_path: Path) -> None:
    """Plot MNIST batch-size trends to mirror the BP_New visual style."""
    colors = {"acc": "#FF6B6B", "gap": "#4ECDC4", "time": "#45B7D1"}
    fig = plt.figure(figsize=(18, 8))
    gs = fig.add_gridspec(2, 3, height_ratios=[2, 1], hspace=0.35, wspace=0.3)

    ax_top = fig.add_subplot(gs[0, :])
    ax_top.plot(results_df["batch_size"], results_df["final_test_acc"], marker="o", lw=2.5, color=colors["acc"])
    ax_top.set_xscale("log")
    ax_top.set_xlabel("Batch Size")
    ax_top.set_ylabel("Test Accuracy (%)")
    ax_top.set_title("MNIST: Test Accuracy vs Batch Size")
    ax_top.grid(True, which="both", ls="--", alpha=0.4)

    ax_gap = fig.add_subplot(gs[1, 0])
    ax_gap.plot(results_df["batch_size"], results_df["generalization_gap"], marker="s", lw=2.0, color=colors["gap"])
    ...
    ax_time.plot(results_df["batch_size"], results_df["avg_epoch_time"], marker="^", lw=2.0, color=colors["time"])
    ...
    if batches:
        chosen = (
            [batches[0]] if len(batches) == 1 else [batches[0], batches[len(batches) // 2], batches[-1]]
        )
        for bs in chosen:
    ...
    plt.suptitle("MNIST Batch Size Sweep", fontsize=16, fontweight="bold")
...
def run_mnist_sweep():
    ...
    results_rows = []
    histories = {}
    reports_dir = OUTPUT_DIR / "mnist_reports"
    reports_dir.mkdir(exist_ok=True)

    print(f"Device: {DEVICE}")
    print(f"Running MNIST sweep for batches {batch_sizes}")

    for bs in batch_sizes:
        print(f"\n=== Batch Size {bs} ===")
        lr = base_lr * (bs / base_batch)
        warmup = 5 if (bs >= 256 and DEVICE == "cuda") else 0
        ...
        report_path = reports_dir / f"mnist_bs_{bs}_report.txt"
    ...
    csv_path = OUTPUT_DIR / "mnist_batchsize_results.csv"
    results_df.to_csv(csv_path, index=False)
    print(f"\nSaved metrics CSV to {csv_path.resolve()}")

    plot_path = OUTPUT_DIR / "mnist_batchsize_summary.png"
    plot_mnist_results(results_df, histories, plot_path)
